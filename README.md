...
This is a nagios plugin (or it can be run as a standalone script) that will monitor the total traffic over an interface. Interface stats reset when a system reboots. To account for this, the script will record the interface traffic to a per-host/interface JSON state file after each run, and sum between reboots. State updates are written with atomic replace, so concurrent checks for different host/interface pairs do not share one database file. This should only be used as a guide, and may not be an accuate representation of actual traffic passing across the wire.<br>
<br>
